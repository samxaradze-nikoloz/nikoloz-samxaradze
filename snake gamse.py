...
pygame.init()
pygame.display.set_caption("ratis trakis chia")
pygame.font.init()
random.seed()


# sichkare da basic ragaceebi

SPEED = 0.36
SNAKE_SIZE = 9
APPLE_SIZE = SNAKE_SIZE     # wgveli da sawmeli erti zoms

SEPARATION = 10    # pixelebis dashorena
SCREEN_HEIGHT = 1000
SCREEN_WIDTH =1000
FPS = 25
...
def checkCollision(posA,As ,posB , Bs):    # A ebi ro iko a ebis toli da b eni b si
    if(posA.x < posB.x+Bs and posA.x+As > posB.x and posA.y < posB.y+Bs and posA.y+As > posB.y):
        return True
    return False

# gvelma ro ekranze tavidan da tavidan iaros

def checkLimits(snake):
    if(snake.x > SCREEN_WIDTH):
        snake.x = SNAKE_SIZE
    if(snake.x < 0):    # ro gavides infitze aket ikit
        snake.x = SCREEN_WIDTH - SNAKE_SIZE
    if(snake.y > SCREEN_HEIGHT):
        snake.y = SNAKE_SIZE
    if(snake.y < 0):   # es naxevarze
        snake.y = SCREEN_HEIGHT - SNAKE_SIZE

# gvelis sawmeli vashli ikoos
...
class Apple:
    def __init__(self, x ,y,state):
        ...
        self.y = y
        self.state = state
        self.color = pygame.color.Color("orange")     # sawmlis feri

    def draw(self,screen):
        pygame.draw.rect(screen,self.color,(self.x,self.y,APPLE_SIZE,APPLE_SIZE),0)

class segment:
    # zemot ro iaros gvelma
...
# yvelaze rtuli modzraoba
    ...
    def getHead(self):    # gvelis tavi
        return(self.stack[0])   #indexi su 0 ikneba

    # exa esaa ro swams da gaizrdeba mere
    ...
    def get_rect(self):     # otxkutxedis forma ro mivce
        rect = (self.x , self.y)
        return rect
    ...
    # ws ro waago ra daejaxeba buum waage
    ...
    # ramac nervebi mishala gvelis xatva
    def draw(self,screen):
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            ...
            # ro gamoxvide
                ...
                return "exit"
            # tugvinda ro tavidan daviwkot
            elif event.key == pygame.K_y:
                return "yes"
            # tu agar gvinda tamasi
...
def main():
    ...
    #gvelis dakeneba (cudad gavxdi)

    mySnake = snake(SCREEN_WIDTH/2,SCREEN_HEIGHT/2)
    mySnake.setDirection(KEY["UP"])
    mySnake.move()
    start_segments = 3   # tavidan 3 nawilian wuwulikad iwkeb
    ...
    # sawmeli
    max_apples = 1  # 1 vashls ro wams gveli 1 jerze
    eaten_apple = False   # ro shewams vashls vashli gakreba
